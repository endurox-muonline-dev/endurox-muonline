 ...
 * 
 */
void JOINSERVER_SVC(TPSVCINFO *p_svc_)
{
	int ret = EXSUCCEED;
	...
	tplogprintubf(log_debug, (char*)"	JOINSERVER_SVC:", p_ub);
...
int tpsvrinit(int argc_, char **argv_)
{
	...
	if (EXSUCCEED != tpadvertise_full((char*)"JOINSERVER", JOINSERVER_SVC, (char*)"JOINSERVER"))
	{
		TP_LOG(log_error, "	Failed to advertise 'JOINSERVER': %s", tpstrerror(tperrno));
		return EXFAIL;
	}
